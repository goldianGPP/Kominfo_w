...
class ModelAbsen extends CI_Model
{
    ...
    {
        $this->db->select(' id_absen, id_pengguna, absen.tgl_presensi, status, is_libur');
        $this->db->from('absen');
        $this->db->join('detail_tgl', 'absen.tgl_presensi=detail_tgl.tgl_presensi', 'right');
...
(select adddate('1970-01-01',t4.i*10000 + t3.i*1000 + t2.i*100 + t1.i*10 + t0.i) tgl_presensi from
 ...
 (select 0 i union select 1 union select 2 union select 3 union select 4 union select 5 union select 6 union select 7 union select 8 union select 9) t4) v");
    ...
    public function getAbsen2($id_pengguna, $year, $month)
    {
        $this->db->select(' id_absen, id_pengguna, absen.tgl_presensi, status, is_libur');
        $this->db->from('absen');
        $this->db->join('detail_tgl', 'absen.tgl_presensi=detail_tgl.tgl_presensi');
    ...
    public function checkAbsen($data)
    {
        $this->db->select('count(id_absen) "rows"');
        $this->db->from('absen');
